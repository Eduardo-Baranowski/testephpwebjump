    ...
    <ul class="product-list">
      <li>
        ...
        <div class="product-image">
            ...
            include('../repositories/conexao_bd.php');
            require_once('../repositories/product_bd.php');
            $imagens = listaImagemProduto($conexao, $produto['id']);
            ?>
            <?php foreach($imagens as $imagem): ?>
                <?php
                    $tipo = $imagem['arquivo_tipo'];
                    $conteudo = $imagem['arquivo_conteudo'];
                ?>
            <div>

                <?php
                    if($conteudo != null){
                        echo '<img src="data:image/gif;base64,' . base64_encode($conteudo) . '" />';

                    } ?>
                <div class="product-info">
                    <div class="product-name"><span><?= $produto['nome']; ?></span></div>
                    <div class="product-price"><span class="special-price">9 available</span> <span><?= $produto['preco']; ?></span></div>
                </div>
                        <div class="product-image">
                            <img src='images/product/tenis-2d-shoes.png' layout='responsive' width='164' height='145' alt='Tênis Basket Light' />
                        </div>
                    <div class="product-info">
                        <div class="product-name"><span><?= $produto['nome']; ?></span></div>
                        <div class="product-price"><span class="special-price">9 available</span> <span><?= $produto['preco']; ?></span></div>
